...
export default async function ContactPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      {/* Hero */}
      <section className="relative bg-[#faf8f5] text-[#6A311D] pt-40 pb-20 px-6 md:px-12 overflow-hidden">
        ...
        <div className={`relative z-10 max-w-7xl mx-auto ${isAr ? 'text-right' : ''}`}>
          ...
          <h1 className="font-headline text-5xl md:text-7xl text-[#6A311D] leading-tight">
            {isAr ? 'دعنا نبدأ' : "Let's Start"} <br />
            <span className="text-[#d4af37] italic font-light">{isAr ? 'محادثة' : 'a Conversation'}</span>
          </h1>
          <p className="text-[#6A311D]/60 mt-4 max-w-xl text-sm md:text-base leading-relaxed">
            {isAr
      ...
      {/* Contact Content */}
      <section className={`py-24 px-6 md:px-12 bg-[#faf8f5] text-[#6A311D] relative overflow-hidden ${isAr ? 'rtl' : ''}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
            ...
            <div className="flex flex-col justify-center gap-10">
              ...
              <div className={`flex gap-6 items-start group ${isAr ? 'flex-row-reverse' : ''}`}>
                <div className="w-14 h-14 shrink-0 rounded-full border border-[#6A311D]/10 flex items-center justify-center group-hover:border-[#d4af37] group-hover:bg-[#d4af37]/10 transition-all duration-300">
                  <span className="material-symbols-outlined text-[#d4af37] text-[22px]">location_on</span>
                </div>
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6A311D]/50 mb-1">{isAr ? 'زيارة معرضنا' : 'Visit Our Showroom'}</p>
                  <p className="text-[#6A311D] font-medium text-sm md:text-base">
                  ...
                  <p className="text-[#6A311D]/50 text-xs mt-1">{isAr ? 'السبت – الخميس، ١٠ص – ٨م' : 'Sat – Thu, 10 AM – 8 PM'}</p>
                </div>
              </div>
              ...
              <div className={`flex gap-6 items-start group ${isAr ? 'flex-row-reverse' : ''}`}>
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6A311D]/50 mb-1">{isAr ? 'اتصل بنا' : 'Call Us'}</p>
                  <a href="tel:01100080609" className="text-[#6A311D] font-medium text-sm md:text-base hover:text-[#d4af37] transition-colors block" dir="ltr">01100080609</a>
                  <a href="tel:01020909498" className="text-[#6A311D] font-medium text-sm md:text-base hover:text-[#d4af37] transition-colors block" dir="ltr">01020909498</a>
                  <p className="text-[#6A311D]/50 text-xs mt-1">{isAr ? 'نرد على مكالماتك خلال ٢٤ ساعة' : 'We respond within 24 hours'}</p>
                </div>
              </div>

              {/* WhatsApp */}
              <div className={`flex gap-6 items-start group ${isAr ? 'flex-row-reverse' : ''}`}>
                <div className="w-14 h-14 shrink-0 rounded-full border border-[#6A311D]/10 flex items-center justify-center group-hover:border-[#25D366] group-hover:bg-[#25D366]/10 transition-all duration-300">
                  <img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" className="w-6 h-6" alt="WhatsApp" />
                </div>
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6A311D]/50 mb-1">WhatsApp</p>
                  <a href="https://wa.me/201100080609" target="_blank" rel="noopener noreferrer"
                    className="text-[#6A311D] font-medium text-sm md:text-base hover:text-[#25D366] transition-colors" dir="ltr">
                    +20 1100080609
                  </a>
                  <p className="text-[#6A311D]/50 text-xs mt-1">{isAr ? 'تحدث معنا مباشرة' : 'Chat with us directly'}</p>
                </div>
              </div>
              ...
              <div className={`flex gap-6 items-start group ${isAr ? 'flex-row-reverse' : ''}`}>
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6A311D]/50 mb-1">{isAr ? 'راسلنا' : 'Email Us'}</p>
                  <a href="mailto:sales@crystalblinds.com" className="text-[#6A311D] font-medium text-sm md:text-base hover:text-[#d4af37] transition-colors">sales@crystalblinds.com</a>
                  <p className="text-[#6A311D]/50 text-xs mt-1">{isAr ? 'للمشاريع الخاصة والتعاون' : 'For bespoke projects & partnerships'}</p>
                </div>
              </div>

              {/* Social */}
              <div>
                <p className={`text-[10px] font-semibold uppercase tracking-widest text-[#6A311D]/50 mb-3 ${isAr ? 'text-right' : ''}`}>{isAr ? 'تابعنا' : 'Follow Us'}</p>
                <div className={`flex gap-3 ${isAr ? 'flex-row-reverse' : ''}`}>
                  {[
                    ...
                    <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer"
                      className="w-10 h-10 rounded-full border border-[#6A311D]/15 flex items-center justify-center hover:bg-[#C6AB8E] hover:border-[#6A311D] transition-all duration-300 group">
            ...
            <div className="relative">
              <div className="bg-white/60 border border-white/40 rounded-2xl p-8 md:p-12 backdrop-blur-xl shadow-[0_20px_40px_rgba(38,23,12,0.05)]">
                <h2 className="font-headline text-2xl text-[#6A311D] mb-8">{isAr ? 'أرسل رسالة' : 'Send a Message'}</h2>
                <form className="flex flex-col gap-8">
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="text" id="cname" placeholder={isAr ? 'أدخل اسمك' : 'Your name'}
                      className="bg-transparent border-b border-[#6A311D]/10 pb-3 text-[#6A311D] placeholder:text-[#6A311D]/30 focus:outline-none focus:border-[#d4af37] transition-colors text-sm" />
                  </div>
                  <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <textarea id="cmsg" rows={4} placeholder={isAr ? 'أخبرنا عن مشروعك...' : 'Tell us about your space and vision...'}
                      className="bg-transparent border-b border-[#6A311D]/10 pb-3 text-[#6A311D] placeholder:text-[#6A311D]/30 focus:outline-none focus:border-[#d4af37] transition-colors text-sm resize-none" />
                  </div>
                  <a href="https://wa.me/201100080609"
                    target="_blank" rel="noopener noreferrer"
                    className={`mt-4 flex items-center justify-center gap-3 bg-[#C6AB8E] text-[#6A311D] py-4 rounded font-bold tracking-widest uppercase text-xs hover:bg-[#3d2b1f] transition-colors shadow-lg`}>
